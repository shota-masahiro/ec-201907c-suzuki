...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	/**
	 * 特定のリクエストに対して「セキュリティ設定」を無視する設定など全体にかかわる設定をする.
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.formLogin() // ログインに関する設定
		    .loginPage("/loginUser") // ログイン画面に遷移させるパス(ログイン認証が必要なパスを指定し、かつログインされていないと、このパスに遷移される)
		    .loginProcessingUrl("/loginUser/login") // ログインボタンを押した際に遷移させるパス(ここに遷移させれば自動的にログインが行われる)
		...
		http.logout() // ログアウトに関する設定
		    ...
		    .logoutSuccessUrl("/loginUser") // ログアウト後に遷移させるパス(ここではログイン画面を指定)
